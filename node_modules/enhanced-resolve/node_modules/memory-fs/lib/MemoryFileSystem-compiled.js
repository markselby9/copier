...
MemoryFileSystem.prototype.statSync = function (_path) {
	...
	for (var i = 0; i < path.length - 1; i++) {
		if (!isDir(current[path[i]])) throw new Error("Path doesn't exist '" + _path + "'");
	...
	} else throw new Error("Path doesn't exist '" + _path + "'");
};
...
MemoryFileSystem.prototype.readFileSync = function (_path, encoding) {
	...
	if (!isFile(current[path[i]])) {
		if (isDir(current[path[i]])) throw new Error("Cannot readFile on directory '" + _path + "'");else throw new Error("Path doesn't exist '" + _path + "'");
	}
	current = current[path[i]];
...
MemoryFileSystem.prototype.readdirSync = function (_path) {
	...
	if (!isDir(current[path[i]])) {
		if (isFile(current[path[i]])) throw new Error("Cannot readdir on file '" + _path + "'");else throw new Error("Path doesn't exist '" + _path + "'");
	}
...
MemoryFileSystem.prototype.mkdirpSync = function (_path) {
	...
	for (var i = 0; i < path.length; i++) {
		if (isFile(current[path[i]])) throw new Error("Path is a file '" + _path + "'");else if (!isDir(current[path[i]])) current[path[i]] = { "": true };
		current = current[path[i]];
	}
	return;
};
...
MemoryFileSystem.prototype.mkdirSync = function (_path) {
	...
	if (isDir(current[path[i]])) throw new new Error("Directory already exist '" + _path + "'")();else if (isFile(current[path[i]])) throw new Error("Cannot mkdir on file '" + _path + "'");
	current[path[i]] = { "": true };
	return;
};
...
MemoryFileSystem.prototype._remove = function (_path, name, testFn) {
	...
	if (!testFn(current[path[i]])) throw new Error("'" + name + "' doesn't exist '" + _path + "'");
...
MemoryFileSystem.prototype.writeFileSync = function (_path, content, encoding) {
	...
	if (isDir(current[path[i]])) throw new Error("Cannot writeFile on directory '" + _path + "'");
...
//# sourceMappingURL=MemoryFileSystem-compiled.js.map